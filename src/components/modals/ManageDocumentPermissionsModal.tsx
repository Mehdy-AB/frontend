...
import { X, Users, Shield, User as UserIcon, Plus, Trash2, ChevronRight, ChevronDown, Settings, FileText } from 'lucide-react';
import { documentService } from '@/api/services/documentService';
import { DocumentPermissionReq, UserDto, RoleDto, GroupDto } from '@/types/api';
...
export default function ManageDocumentPermissionsModal({
  isOpen,
  onClose,
  documentId,
  documentName,
  onSuccess
}: ManageDocumentPermissionsModalProps) {
  ...
  const {
    ...
    fetchFunction: async (currentPage, searchTerm) => {
      if (selectedTab === 'user') {
        return await documentService.getAvailableUsersForDocument(documentId, {
          page: currentPage,
          size: pageSize,
          search: searchTerm || undefined
        });
      } else if (selectedTab === 'role') {
        return await documentService.getAvailableRolesForDocument(documentId, {
          page: currentPage,
          size: pageSize,
          search: searchTerm || undefined
        });
      } else {
        return await documentService.getAvailableGroupsForDocument(documentId, {
          page: currentPage,
          size: pageSize,
          search: searchTerm || undefined
        });
  ...
  const fetchCurrentPermissions = async () => {
    try {
      const response = await documentService.getDocumentShared(documentId, { page: 0, size: 1000 });
  ...
  const handleAddPermission = async (entity: UserDto | RoleDto | GroupDto) => {
    try {
      setLoading(true);
      await documentService.createOrUpdateDocumentShared(documentId, {
  ...
  const handleUpdatePermission = async (perm: CurrentPermission, newPermission: DocumentPermissionReq) => {
    try {
      setLoading(true);
      await documentService.createOrUpdateDocumentShared(documentId, {
  ...
  const handleRemovePermission = async () => {
    ...
    try {
      setLoading(true);
      await documentService.deleteDocumentShared(documentId, permissionToDelete.entity.id);
  ...
  const getPresetName = (permission: DocumentPermissionReq): keyof typeof PERMISSION_PRESETS | 'custom' => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-blue-600 to-purple-600">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Manage Document Permissions</h2>
              {documentName && <p className="text-sm text-white/80">Document: {documentName}</p>}
            </div>
          </div>
          <button
            onClick={onClose}
                        ...
                        <div className="border-t border-gray-200 bg-gray-50 p-4">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {['canView', 'canUpload', 'canEdit', 'canDelete', 'canShare', 'canManagePermissions'].map((key) => (
                                <label key={key} className="flex items-center gap-2 text-sm">
                                  <input
                                    type="checkbox"
                                    checked={perm.permission[key as keyof DocumentPermissionReq] as boolean}
