...
/**
 * CancelNode - Forced workflow cancellation
 * Backend: CANCEL type, CancelNodeHandler
 */
const CancelNode = ({ data, selected, id }: CancelNodeProps) => {
    return (
        <div
        ...
        >
            ...
            <div className="flex items-center gap-2">
                ...
                <div>
                    <div className="font-bold text-sm text-gray-900">{data.label || 'Cancel'}</div>
                    <div className="text-[10px] text-gray-400 uppercase tracking-wider">Terminate</div>
                </div>
            </div>

            {data.reason && (
                <div className="text-xs text-red-600 bg-red-50 px-2 py-1 rounded mt-2 truncate">
                    {data.reason}
                </div>
            )}
        </div>
...
export default memo(CancelNode);
